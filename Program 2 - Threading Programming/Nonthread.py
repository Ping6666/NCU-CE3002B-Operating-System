...
def downloadByLoop(downloadList, fileBase, preUrl):
    totalCount = 0
    for bookList in downloadList:
        for i, pageInfo in enumerate(bookList):
            ...
            totalCount += 1
            downloadPic(pageInfo, fileBase, preUrl)
    return
...
def mergeByLoop(downloadList, bookBase):
    for i, bookList in enumerate(downloadList):
        ...
        picToPDF(bookBase, bookSubName, pageNumber)
    return


def startWebCrawler(bookNum, PDFmerge):
    # pre stage
    ...
    stage4 = time.time()
    downloadByLoop(totalList, dirBase, urlOrg)
    stage4_ = time.time()
    ...
    if PDFmerge:
        ...
        stage5 = time.time()
        mergeByLoop(totalList, bookBase)
        stage5_ = time.time()
...
def main(argv, bookNumInput):
    outputStr = 'Nonthread.py [-h][-p] -b <bookName>'
    helpStr = 'Nonthread.py\n-h : for this message.\n-b <bookName> : for input book name on the url.\n-p : (optional) for enable merging and converting to PDF.'
    try:
        opts, args = getopt.getopt(argv, "hpb:", ["bookNum="])
    ...
    if checkNum == 1:
        startWebCrawler(bookNum, checkPDF)
    else:
        print(outputStr)
    return
